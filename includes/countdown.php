 ...
 * @return void
 */
function affwp_theme_enqueue_countdown_scripts() {

	// Register countdown script.
	wp_register_script( 'countdown', get_stylesheet_directory_uri() . '/js/jquery.countdown.min.js', array( 'jquery' ), AFFWP_THEME_VERSION );

	wp_register_script( 'moment-timezone', get_stylesheet_directory_uri() . '/js/moment-timezone-with-data.min.js', array(), AFFWP_THEME_VERSION );
	wp_register_script( 'moment', get_stylesheet_directory_uri() . '/js/moment.min.js', array(), AFFWP_THEME_VERSION );

	// Only enqueue script if a sale notice is published.
	if ( affwp_theme_sale_notice_active() ) {
...
add_action( 'wp_enqueue_scripts', 'affwp_theme_enqueue_countdown_scripts' );
 ...
 */
function affwp_theme_sale_notice_active() {

	$args = array(
		'posts_per_page'   => -1,
		'meta_key'         => 'affwp_notice_is_sale',
		'meta_value'       => true,
		'post_type'        => 'notices',
		'post_status'      => 'publish',
	);
 ...
 * @return $content
 */
function affwp_theme_countdown_shortcode( $atts, $content = null ) {
...
add_shortcode( 'countdown', 'affwp_theme_countdown_shortcode' );
